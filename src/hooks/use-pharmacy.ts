    ...
    /**
     * 🔄 Hook para carregar medicamentos
    ...
    const query = useQuery({
        ...
        try {
            ...
            // Se houver erro, retorna mock
            if (error) {
            console.warn('⚠️ Usando mock data (tabela não existe)');
            return medications as Medication[];
            }

            // Retorna dados reais ou mock se vazio
            return ((data && data.length > 0 ? data : medications) as Medication[]);
        } catch (err) {
            console.warn('📦 Usando mock data (erro na query)', err);
            return medications as Medication[];
        }
        },
    ...
    return {
        medications: (query.data as Medication[]) || medications,
